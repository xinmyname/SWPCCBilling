...
namespace SWPCCBilling.Infrastructure
{
    public class InvoiceStore
    {
        ...
        public InvoiceStore(ILog log, DatabaseFactory dbFactory, SettingsStore settingsStore)
        {
            _log = log;
        ...
        public Invoice Load(DateTime date, long familyId, string familyName)
        {
        	...
        	amountDue = con.ExecuteScalar<float>("SELECT Amount FROM Invoice WHERE FamilyId=? AND Date=?",
            	                               new { familyId, month });

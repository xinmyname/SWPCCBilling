	...
	public class EmbeddedStaticContentConventionBuilder
	{
		...
		private static Func<string, Func<Response>> BuildContentDelegate(NancyContext context, string requestedPath, string contentPath, Assembly assembly, string[] allowedExtensions)
		{
			return requestPath =>
			{
				...
				var fileName =
					Path.GetFullPath(Path.Combine("/", transformedRequestPath));

				var contentRootPath =
					Path.GetFullPath(Path.Combine("/", GetEncodedPath(contentPath)));
				...
				var resourceName =
					Path.GetDirectoryName(assembly.GetName().Name + fileName).Replace('/', '.');
			
				fileName =
					Path.GetFileName(fileName);

				string[] resourceNames = assembly.GetManifestResourceNames();
				string finalResourceName = resourceName + "." + fileName;

				if (!resourceNames.Any(x => string.Equals(x, finalResourceName, StringComparison.OrdinalIgnoreCase)))
				{
